...
const tests_egep5 = {
  ...
  renderizarItemLikert(numero, texto, indice, grupo) {
    ...
    opciones.forEach((opcion, valor) => {
      html += `<label class="likert-option"><input type="radio" name="${numero}" value="${valor}" onchange="tests_egep5.cambiarRespuesta('${grupo}', ${indice}, ${valor})"><span class="likert-label">${opcion}</span></label>`;
    });
    html += `</div></div>`;
    return html;
  },
  ...
  renderizarFuncionamiento() {
    ...
    this.funcionamientoDefinitions.forEach((def, indice) => {
      html += `<div class="functioning-item"><label class="checkbox-item"><input type="checkbox" name="item_${52 + indice}" onchange="tests_egep5.cambiarFuncionamiento(${indice}, this.checked)"><span>${52 + indice}. ${def}</span></label></div>`;
    });
