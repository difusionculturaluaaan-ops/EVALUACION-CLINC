...
const app = {
  ...
  /**
   * Export/Import JSON para tests
   */
  exportarJSONTest(tipoTest) {
    const respuestasKey = tipoTest === 'SCL90R' ? 'scl_responses' :
                         tipoTest === 'HAMILTON' ? 'ham_responses' :
                         tipoTest === 'PCLR' ? 'pclr_responses' :
                         tipoTest === 'ISRA' ? 'isra_responses' :
                         tipoTest === 'TDS' ? 'tds_responses' :
                         tipoTest === 'EGEP5' ? 'egep5_responses' : null;

    if (!respuestasKey) {
      this.mostrarToast('❌ Tipo de test no soportado', 'error');
      return;
    }

    const respuestas = JSON.parse(localStorage.getItem(respuestasKey) || '{}');
    if (Object.keys(respuestas).length === 0) {
      this.mostrarToast('⚠️ No hay respuestas para exportar', 'warning');
      return;
    }
    ...
    a.download = `${tipoTest}_${new Date().toISOString().split('T')[0]}.json`;
    a.click();

    this.mostrarToast('✓ JSON exportado correctamente', 'success');
  },

  importarJSONTest(tipoTest) {
    const inputId = `${tipoTest.toLowerCase()}-file-import`;
    document.getElementById(inputId)?.click();
  },

  manejarImportJSON(evt, tipoTest) {
    ...
    reader.onload = e => {
      try {
        ...
        if (data.tipoTest !== tipoTest) {
          this.mostrarToast(`❌ Este JSON es para ${data.tipoTest}, no para ${tipoTest}`, 'error');
          return;
        }
        ...
        if (respuestasKey) {
          localStorage.setItem(respuestasKey, JSON.stringify(data.respuestas));
          this.mostrarToast('✓ JSON importado correctamente', 'success');

          // Recargar el test para mostrar las respuestas
          setTimeout(() => location.reload(), 500);
        }
      } catch (error) {
        this.mostrarToast(`❌ Error al importar: ${error.message}`, 'error');
      }
    };
