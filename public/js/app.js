...
const app = {
  ...
  async cargarTestsHabilitados() {
    try {
      ...
      document.querySelectorAll('[data-page="scl90r"], [data-page="hamilton"], [data-page="mmpi-pro"], [data-page="cuida"], [data-page="isra"], [data-page="egep5"], [data-page="tds3"], [data-page="pclr"], [data-page="scid2"], [data-page="mbi"], [data-page="cisneros"]').forEach(btn => {
  ...
  setupAuth() {
    ...
    try {
      ...
      // Cargar tests habilitados para el tenant (ya se ejecutó en init())
      // La función cargarTestsHabilitados() se llamó automáticamente en init()
  ...
  /**
   * Iniciar TDS-3 (Nuevo - versión mejorada)
   */
  iniciarTDS3() {
    if (!this.pacienteActivo) {
      this.mostrarToast('Primero debes crear o seleccionar un paciente', 'error');
    ...
    // Guardar datos del paciente para que el micrositio TDS-3 pueda acceder
    localStorage.setItem('paciente_nombre', nombrePaciente);
    localStorage.setItem('paciente_id', this.pacienteActivo.id);
    sessionStorage.setItem('paciente_nombre', nombrePaciente);
    sessionStorage.setItem('pacienteSeleccionado', this.pacienteActivo.id);
    ...
    window.location.href = `/micrositios/tds3/?paciente_id=${this.pacienteActivo.id}&token=${encodeURIComponent(token)}`;
  },
    ...
    try {
      ...
      escalas.forEach((esc, idx) => {
        ...
        for (let i = 0; i <= 4; i++) {
          ...
          svg.push(`<line x1="${xPos}" y1="${y + 2}" x2="${xPos}" y2="${y + barHeight - 2}" stroke="#dc2626" stroke-width="2.5"/>`);
        }
